...
### VARIABLE BLOCK ###
######################
DOWNLOAD_URL="https://e3.boxcdn.net/box-installers/desktop/releases/mac/Box.pkg"
DOWNLOAD_LOCATION=/tmp/box.pkg
...
download_app(){
  ...
  if [[ -e "$DOWNLOAD_LOCATION" ]] && [[ $curlExitCode == 0 ]]; then
    ...
    send_user_message "FAILED: Could not download box drive"
    exit 1
  fi
}
